...
def maskedfill_fun(ntype):
    ...
    mask = Tensor(np.array([[True, True, False, True], [False, False, True, False]]).astype(bool))
    ...
    mask = Tensor(np.array([[True, True, True, True], [True, True, True, True]]).astype(bool))
    value = Tensor(np.array(1).astype(ntype))
    expect = np.array([[1, 1, 1, 1], [1, 1, 1, 1]]).astype(ntype)
    output = maskedfill_net(inputs, mask, value)
    assert (output.asnumpy() == expect).all()

    mask = Tensor(np.array([[False, False, False, False], [False, False, False, False]]).astype(bool))
    value = Tensor(np.array(22).astype(ntype))
    ...
    # BroadCast
    mask = Tensor(np.array([True, True, False, True]).astype(bool))
    value = Tensor(np.array(22).astype(ntype))
